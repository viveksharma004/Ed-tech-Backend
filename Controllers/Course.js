...
exports.showAllCourses=async (req,res)=>{
    try{
        const allCourses=await Course.find({},{
            ...
            instructor:true,
        }).populate(["instructor","ratingAndReview","Category"]).populate().exec();
...
exports.getCourseDetails=async (req,res)=>{
    try{
        ...
        if(!courseDetails){                                                               
            return res.status(404).json({
                success:false,
                message:`Course details not found for this ${courseId}`
            })
        }
    ...
    const totalDuration = convertSecondsToDuration(totalDurationInSeconds)
        ...
        console.log("Error during the getting course details",err.message);
        return res.status(500).json({
            success:false,
      ...
      for (const key in updates) {
        if (updates.hasOwnProperty(key)) {
          if (key === "tag" || key === "instructions") {
            course[key] = JSON.stringify(JSON.parse(updates[key]))
          } else {
